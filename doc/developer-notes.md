...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin Core](#general-dyno-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play dynos" on the test network, if you
...
([`valgrind.supp`](https://github.com/dynamopay/dyno/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_dyno
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_dyno --log_level=test_suite
$ valgrind -v --leak-check=full src/dynod -printtoconsole
```
...
# A coverage report will now be accessible at `./test_dyno.coverage/index.html`.
```
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/dynamopay/dyno/issues/12691)
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `DYNO_FOO_BAR_H`, e.g.

```c++
#ifndef DYNO_FOO_BAR_H
#define DYNO_FOO_BAR_H
...
#endif // DYNO_FOO_BAR_H
```
...
- src/libsecp256k1
  - Upstream at https://github.com/dyno-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/dyno-core/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:dyno/dyno.git
...
Commit [`bb81e173`](https://github.com/dynamopay/dyno/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `dyno-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `dyno-cli` and the GUI debug console use this table to determine how to
