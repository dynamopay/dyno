...
###   dynamopay.io and dyno.org and compares them.
...
WORKINGDIR="/tmp/dyno_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://dynamopay.io"
HOST2="https://dyno.org"
BASEDIR="/bin/"
VERSIONPREFIX="dyno-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'dyno-',
...
if [ $? -ne 0 ]; then
   echo "dyno.org failed to provide signature file, but dynamopay.io did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "dyno.org and dynamopay.io signature files were not equal?"
