...
const char * const BITCOIN_CONF_FILENAME = "dyno.conf";
const char * const BITCOIN_PID_FILENAME = "dynod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dyno" is a composite category enabling all Dyno-related debug output
            if(ptrCategory->count(string("dyno"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DynoCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DynoCore
    // Mac: ~/Library/Application Support/DynoCore
    // Unix: ~/.dynocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DynoCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DynoCore";
#else
    // Unix
    return pathRet / ".dynocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dyno.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dyno.conf
